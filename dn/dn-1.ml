...
let rec collatz stevilo = 
  if stevilo = 1 then [1] 
  else stevilo :: collatz (if stevilo mod 2 = 0 then stevilo / 2 else 3 * stevilo + 1)
...
let fiksne_tocke funkcija tocke = 
  List.filter (fun x -> funkcija x = x) tocke 
...
let  rec sep_concat locilo seznami = 
  match seznami with
  | [] -> [locilo]
  | prvi :: ostali -> [locilo] @ prvi @ sep_concat locilo ostali 
