...
def globina {A : Type} : tree A → Nat :=
  fun t =>
    match t with
    | .empty => 0
    | .node _ l d => 1 + Nat.max (globina l) (globina d)

def zrcali {A : Type} : tree A → tree A :=
  fun t =>
    match t with
    | .empty => .empty
    | .node x l d => .node x (zrcali d) (zrcali l)
...
def zberi {A : Type} : tree A → List A :=
  fun t =>
    match t with
    | .empty => []
    | .node x l d => stakni (zberi l) (stakni [x] (zberi d))
...
def velikost {A : Type} : tree A → Nat :=
  fun t =>
    match t with
    | .empty => 0
    | .node _ l d => 1 + (velikost l) + (velikost d)
